...
# ========= Google Drive Setup ========= #
# Make sure SERVICE_ACCOUNT_JSON env var exists in Render
try:
    service_account_info = json.loads(os.environ["SERVICE_ACCOUNT_JSON"])
except KeyError:
    raise RuntimeError("SERVICE_ACCOUNT_JSON environment variable not set in Render!")

credentials = service_account.Credentials.from_service_account_info(service_account_info)
drive_service = build('drive', 'v3', credentials=credentials)

# Optional: Upload to a specific folder in Drive
# Set DRIVE_FOLDER_ID env var in Render if desired, else leave None
drive_folder_id = os.environ.get("DRIVE_FOLDER_ID", None)

def upload_to_drive(file_path, file_name):
    """Upload local file to Google Drive, return file_id or None if failed."""
    try:
...
async def handle_download(magnet):
    ...
    # After completion - upload to Google Drive
...
@app.get("/")
def home():
    return {"message": "Torrent Downloader API with Drive Upload + ETA + Auto-Cancel Running!"}
...
async def download_torrent(request: Request):
    ...
    if not magnet:
        return {"error": "Send JSON: {'magnet': 'magnet_link'}"}
...
def get_progress_text():
    ...
    for magnet, info in active_downloads.items():
        output_lines.append(f"Magnet: {magnet}")
